...
export class PersonRepository {
  ...
  @GenerateSql({ params: [DummyValue.UUID, { elevated: false }, DummyValue.UUID] })
  async getStatistics(personGroupId: string, ctx: PolicyContext, userId: string): Promise<PersonStatistics> {
    const result = await this.db
      .selectFrom('asset_face')
      .leftJoin('asset', (join) =>
        join
